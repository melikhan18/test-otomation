...
import WebWorkspaceTree, { type WebSelection } from "@/components/automation/WebWorkspaceTree";
import WebScenarioPanel from "@/components/automation/WebScenarioPanel";
import WebSuitePanel from "@/components/automation/WebSuitePanel";
import {
  webScenarioApi, webSuiteApi, webWorkspaceApi,
  type WebScenarioCreate, type WebSuiteCreate,
} from "@/lib/webAutomation";
import { useAuthStore } from "@/store/auth";

/**
 * Web platform's workspace page — a structural mirror of Android's
 * {@code WorkspacePage}: 280px tree on the left, detail panel on the
 * right, URL-encoded selection so the view is shareable.
 *
 * <p>Same DOM shape and same prop wiring as the Android page; the only
 * differences are the data source (web APIs) and the slightly slimmer
 * detail panels (no DnD reorder, no Inspector, no app prep — see
 * WebScenarioPanel + WebSuitePanel).</p>
 */
export default function WebWorkspacePage() {
  ...
  // Drop ?scenario= / ?suite= when tenancy changes (same hygiene as Android).
  ...
  function setSelection(s: WebSelection) {
  ...
  const treeQ = useQuery({
    queryKey: ["web-workspace-tree", activeCompanyId ?? null],
    queryFn: webWorkspaceApi.tree,
    refetchOnWindowFocus: false,
    enabled: activeCompanyId != null,
  });

  function refreshTree() { qc.invalidateQueries({ queryKey: ["web-workspace-tree"] }); }
  ...
  const createSuite = useMutation({
    mutationFn: (b: WebSuiteCreate) => webSuiteApi.create(b),
    onSuccess: (s) => { refreshTree(); setCreatingSuite(false); setSelection({ kind: "suite", id: s.id }); },
  });
  const createScenario = useMutation({
    mutationFn: async (req: { body: WebScenarioCreate; suiteId?: number }) => {
      const created = await webScenarioApi.create(req.body);
      if (req.suiteId) await webSuiteApi.addScenario(req.suiteId, created.id);
  ...
  // If the selected resource was deleted in the tree, clear selection.
  useEffect(() => {
  ...
  return (
    <>
      <TopBar
        crumbs={[{ label: "Automation", to: "/automation/web" }, { label: "Workspace" }]}
        actions={
          treeQ.data && (
...
}: { busy?: boolean; error?: string; onClose: () => void; onSubmit: (b: WebSuiteCreate) => void }) {
        ...
        <div className="p-5 space-y-4">
          <label className="block">
            ...
            <input autoFocus value={name} onChange={(e) => setName(e.target.value)}
              placeholder="Checkout regression suite" className="input" />
          </label>
          <label className="block">
          ...
          <label className="block">
            ...
            <input value={tagsInput} onChange={(e) => setTagsInput(e.target.value)}
              placeholder="smoke, regression" className="input" />
          </label>
          {error && (
...
function CreateScenarioDialog({
  ...
  onClose: () => void; onSubmit: (b: WebScenarioCreate) => void;
}) {
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [tagsInput, setTagsInput] = useState("");
      ...
      <Card className="w-full max-w-lg">
        ...
        <div className="p-5 space-y-4">
          <label className="block">
            <span className="label block mb-1.5">Name</span>
            <input autoFocus value={name} onChange={(e) => setName(e.target.value)}
              placeholder="Login flow happy path" className="input" />
          </label>
          <label className="block">
            <span className="label block mb-1.5">Description (optional)</span>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="input resize-y" />
