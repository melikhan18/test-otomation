...
import type { WebWorkspaceTree } from "@/lib/webAutomation";
...
type Props = {
  tree: WebWorkspaceTree | undefined;
  selection: WebSelection;
  onSelect: (s: WebSelection) => void;
  onCreateSuite: () => void;
...
/**
 * Visual mirror of {@code WorkspaceTree} (the Android tree). Same DOM
 * shape, same icons, same spacing — the only difference is the data
 * source (web workspace API) so users switching platforms keep their
 * muscle memory.
 */
export default function WebWorkspaceTree({
