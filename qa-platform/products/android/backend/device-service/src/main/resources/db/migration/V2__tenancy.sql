...
UPDATE android_device.devices d
   SET company_id = c.id
  FROM auth.companies c
 WHERE c.legacy_product_id = d.product_id;

CREATE INDEX idx_devices_company ON android_device.devices (company_id);

-- enrollment_tokens are also company-scoped (mints a token for one company's pool).
ALTER TABLE android_device.enrollment_tokens ADD COLUMN company_id BIGINT;
UPDATE android_device.enrollment_tokens t
   SET company_id = c.id
  FROM auth.companies c
 WHERE c.legacy_product_id = t.product_id;

-- Whitelist: when devices.restricted = true, only projects listed here can see
-- and reserve the android_device. restricted=false → all projects in the company see it.
CREATE TABLE android_device.device_project_access (
    device_id  BIGINT NOT NULL REFERENCES android_device.devices(id) ON DELETE CASCADE,
    project_id BIGINT NOT NULL,
    granted_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    granted_by BIGINT,
    PRIMARY KEY (device_id, project_id)
);
CREATE INDEX idx_device_project_access_project ON android_device.device_project_access (project_id);
