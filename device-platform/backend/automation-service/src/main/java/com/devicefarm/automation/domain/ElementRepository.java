...
public interface ElementRepository extends JpaRepository<ElementEntity, Long> {
    Optional<ElementEntity> findByProductIdAndName(Long productId, String name);
    boolean existsByProductIdAndName(Long productId, String name);

    List<ElementEntity> findAllByProductIdOrderByNameAsc(Long productId);
    ...
    @Query("""
           SELECT e FROM ElementEntity e
            ...
            ORDER BY e.name ASC
           """)
    List<ElementEntity> searchByProductId(@Param("productId") Long productId, @Param("q") String q);
}
